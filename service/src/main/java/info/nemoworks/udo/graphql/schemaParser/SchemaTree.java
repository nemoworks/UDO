            ...
            hashMap.forEach((key, value1) -> {
                ...
                switch (typeName) {
                    case Link:
                        ...
                        schemaTree.linkList.add(new Link(key
                    ...
                    case Embedded:
                        ...
                        schemaTree.inputMap.put(key,new TypeName(new GraphQLPropertyConstructor(value1.get("typeName").toString()).inputKeyWordInQuery()));
                        break;
                    case EmbeddedList:
                        schemaTree.typeMap.put(key, new ListType(
                                new TypeName(value1.get("typeName").toString())));
                        schemaTree.inputMap.put(key,new ListType(new TypeName(new GraphQLPropertyConstructor(value1.get("typeName").toString()).inputKeyWordInQuery())));
                        break;
                    case number:
                    ...
                    case object:
                        ...
                        schemaTree.inputMap.put(key,new TypeName(new GraphQLPropertyConstructor(jsonObject.get("name").getAsString()).inputKeyWordInQuery()));
                        schemaTree.filterMap.put(key,new TypeName(new GraphQLPropertyConstructor(jsonObject.get("name").getAsString()).filterKeyWordInQueryXxlist()));
